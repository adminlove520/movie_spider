...
import re
from pyecharts import Line, Geo, Bar, Pie, ThemeRiver,Page
...
def word_cloud(csv_file, stopwords_path, pic_path):
    ...
    wc = WordCloud(width=4667, height=2000, background_color='white',
...
def count_city(csv_file):
    ...
    All_city = jieba.cut("".join(city)) # 分词，将省份和市级地名分开，当然有一些如吉林长春之类没有很好的分开，因此需要用re.sub（）来将之中的省份去除掉
    final_city= []
    for a_city in All_city:
        a_city_sub = re.sub(rep_province,"",a_city) # 对每一个单元使用sub方法，如果有省份的名称，就将其替换为“”（空）
...
def draw_citys_pic(csv_file):
    ...
    geo = Geo("","author",title_pos="center", width=1200,height=600, background_color='#404a59', title_color="#fff")
    ...
    bar.render(csv_file+"_城市评论.html")  # 取CSV文件名的前8位数
    page.add_chart(bar)
    page1.add(bar)
    pie = Pie("", "评论来源饼图", title_pos="center", width=1200, height=600)
    ...
    pie.render(csv_file + "_城市评论.html")  # 取CSV文件名的前8位数
    page.add_chart(pie)
    page1.add(pie)
...
def score_draw(csv_file):
    ...
    page.add_chart(line)
    page1.add(line)
    line = Line("", "日投票量分析", title_pos="center", width=1200, height=600,
              background_color='#404a59', title_color="#fff")
    page.render(csv_file[:-4] + "_日投票量分析汇总.html")

#汇总
    page1.render(csv_file[:-4] + "_影评分析.html")
...
if __name__ == '__main__':
    main("长津湖之水门桥", "stopwords.txt", "temp.jpg" )